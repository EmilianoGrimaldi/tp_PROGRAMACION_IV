...
public class DataLoader implements CommandLineRunner {
    ...
    public DataLoader(UsuarioService usuarioService, CategoriaService categoriaService, ProductoService productoService,
    ...
    public void run(String... args) throws Exception {
        ...
        UsuarioDto u1Dto = usuarioService
                .save(new UsuarioCreate("Juan", "Perez", "juan@mail.com", "11223344", "3123124", Rol.USUARIO));
        Usuario u1 = usuarioRepository.findById(u1Dto.id()).orElseThrow();

        UsuarioDto u2Dto = usuarioService
                .save(new UsuarioCreate("Ana", "Gomez", "ana@mail.com", "55667788", "4214312", Rol.ADMIN));
        ...
        ProductoDto p1Dto = productoService
                .save(new ProductoCreate("Notebook", 1500.0, "Notebook potente", 10, "url1", true, c1));
        Producto p1 = productoRepository.findById(p1Dto.id()).orElseThrow();

        ProductoDto p2Dto = productoService
                .save(new ProductoCreate("Mouse", 25.0, "Mouse optico", 50, "url2", true, c1));
        Producto p2 = productoRepository.findById(p2Dto.id()).orElseThrow();

        ProductoDto p3Dto = productoService
                .save(new ProductoCreate("Teclado", 45.0, "Teclado mecanico", 30, "url3", true, c1));
        Producto p3 = productoRepository.findById(p3Dto.id()).orElseThrow();

        ProductoDto p4Dto = productoService
                .save(new ProductoCreate("Silla", 150.0, "Silla de oficina", 20, "url4", true, c2));
        Producto p4 = productoRepository.findById(p4Dto.id()).orElseThrow();

        ProductoDto p5Dto = productoService
                .save(new ProductoCreate("Mesa", 200.0, "Mesa de comedor", 15, "url5", true, c2));
        Producto p5 = productoRepository.findById(p5Dto.id()).orElseThrow();

        ProductoDto p6Dto = productoService
                .save(new ProductoCreate("Lampara", 30.0, "Lampara LED", 100, "url6", true, c2));
        Producto p6 = productoRepository.findById(p6Dto.id()).orElseThrow();

        ProductoDto p7Dto = productoService
                .save(new ProductoCreate("Remera", 20.0, "Remera de algodon", 200, "url7", true, c3));
        Producto p7 = productoRepository.findById(p7Dto.id()).orElseThrow();

        ProductoDto p8Dto = productoService
                .save(new ProductoCreate("Pantalon", 40.0, "Jean azul", 150, "url8", true, c3));
        Producto p8 = productoRepository.findById(p8Dto.id()).orElseThrow();

        ProductoDto p9Dto = productoService
                .save(new ProductoCreate("Zapatillas", 80.0, "Zapatillas deportivas", 80, "url9", true, c3));
        Producto p9 = productoRepository.findById(p9Dto.id()).orElseThrow();

        ProductoDto p10Dto = productoService
                .save(new ProductoCreate("Gorra", 15.0, "Gorra negra", 50, "url10", true, c3));
        ...
        ped1.setUsuario(u1);
        new PedidoEdit(LocalDate.now(), Estado.PENDIENTE, null, FormaPago.TARJETA).applyTo(ped1);

        DetallePedido dp1_1 = new DetallePedidoCreate(1, p1).toEntity();
        dp1_1.setPedido(ped1);
        ped1.getDetalles().add(dp1_1);

        DetallePedido dp1_2 = new DetallePedidoCreate(2, p2).toEntity();
        dp1_2.setPedido(ped1);
        ped1.getDetalles().add(dp1_2);
        ...
        ped2.setUsuario(u2);
        new PedidoEdit(LocalDate.now(), Estado.CONFIRMADO, null, FormaPago.TRANSFERENCIA).applyTo(ped2);

        DetallePedido dp2_1 = new DetallePedidoCreate(1, p4).toEntity();
        dp2_1.setPedido(ped2);
        ped2.getDetalles().add(dp2_1);

        DetallePedido dp2_2 = new DetallePedidoCreate(4, p7).toEntity();
        dp2_2.setPedido(ped2);
        ped2.getDetalles().add(dp2_2);

        DetallePedido dp2_3 = new DetallePedidoCreate(2, p9).toEntity();
        dp2_3.setPedido(ped2);
        ped2.getDetalles().add(dp2_3);
        ...
        ped3.setUsuario(u1);
        new PedidoEdit(LocalDate.now(), Estado.TERMINADO, null, FormaPago.EFECTIVO).applyTo(ped3);

        DetallePedido dp3_1 = new DetallePedidoCreate(10, p6).toEntity();
        dp3_1.setPedido(ped3);
        ped3.getDetalles().add(dp3_1);

        DetallePedido dp3_2 = new DetallePedidoCreate(1, p3).toEntity();
        dp3_2.setPedido(ped3);
        ped3.getDetalles().add(dp3_2);
